    ...
    function parse() {
        ...
        for(var i=0; i<json.data.Floors.length; i++){
            ...
            for(var j=0; j<floor.Rooms.length; j++){
                ...
                var Rooms2 = floor.Rooms[j];
                Rooms2.rect = IDM.GeomUtil.getBoundingRect(Rooms2.Outline[0][0]);

                if(is3d) {
                    points = parsePoints(Rooms2.Outline[0][0]);
                    shape = new THREE.Shape(points);

                    var center = Rooms2.Center;
                    floorObj.points.push({ name: Rooms2.Name, type: Rooms2.Type, position: new THREE.Vector3(center[0] * scale, floorHeight * scale, -center[1] * scale)});

                    //solid model
                    extrudeSettings = {amount: floorHeight, bevelEnabled: false};
                    geometry = new THREE.ExtrudeGeometry(shape, extrudeSettings);
                    material = new THREE.MeshLambertMaterial(theme.room(parseInt(Rooms2.Type), Rooms2.Category));
                    ...
                    Rooms2.fillColor = theme.room(parseInt(Rooms2.Type), Rooms2.Category).color;
                    Rooms2.strokeColor = theme.strokeStyle.color;
