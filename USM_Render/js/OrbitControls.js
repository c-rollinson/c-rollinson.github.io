...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    function onMouseMove( event ) {
        ...
        if ( state === STATE.ROTATE ) {
            ...
            //  Adjusted code to add the (scale*.95/1.05) values to enable mouse scrollwheel click to zoom functionality again. 
            //  14-1-21 -Conor
            if ( zoomDelta.y > 0 ) {

                scope.zoomIn(scale*0.95);

            } else {

                scope.zoomOut(scale*1.05);
    ...
    function onMouseWheel( event ) {
        ...
        //  Adjusted this mouse section to add (scale */ 1.1/0.9). This undoes the mouse scrollwheel camera view break that occured
        //  when fixing the css zoom in button issue. -Conor 14/1/21
        if ( delta > 0 ) {

            scope.zoomOut(scale*1.1);


        } else {

            scope.zoomIn(scale*0.9);
